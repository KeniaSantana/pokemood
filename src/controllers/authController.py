...
class AuthController:
    ...
    def registrar_usuario(self, nombre, apellido, correo, password, telefono):
        try:
    ...
    def login(self, correo, password):
        try:
            user = self.model.validar_login(correo, password)
            if user:
    ...
    def recuperar_password(self, correo):
        try:
            ...
            success = self.model.actualizar_password(correo, nueva_password)
            if not success:
                return False, "Correo no encontrado"

            enviado = enviar_correo(correo, nueva_password)
            if enviado:
